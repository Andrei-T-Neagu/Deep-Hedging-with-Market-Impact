...
for T in Ts:
    ...
    loss_type = "RSMSE"
    impact_values = [0.025, 0.01, 0.0]
    impact_persistence = [0, 0.7, 1.1, -1] # impact persistence of {1, 0.5, 0.33, 0}
    for persistence in impact_persistence:
        ...
        for i, impact in enumerate(impact_values):
            ...
            agent.model = torch.load("/home/a_eagu/Deep-Hedging-with-Market-Impact/{time_period_folder}/name".format(time_period_folder=time_period, name=name))
            deltas, hedging_err, S_t, V_t, A_t, B_t, = agent.test(test_size=test_size, test_set=test_set)
            S_t_bid = S_t * ((1 + 1 + B_t) ** beta - (1 + B_t) ** beta)
            S_t_bids.append(S_t_bid)
            if i == 0:
                max_error_index = np.argmax(hedging_err)
            global_hedge_deltas.append(deltas[:,max_error_index:max_error_index+1,0])
        ...
        plt.plot(S_t[:,max_error_index:max_error_index+1], label="Stock prices path")
        for i, S_t_bid in enumerate(S_t_bids):
            plt.plot(S_t_bid[:,max_error_index:max_error_index+1], label="Bid prices path with alpha = {:.4f}, beta = {:.4f}".format(1.0-impact_values[i], 1.0+impact_values[i]))
        ...
        plt.savefig("code_pytorch/effect_of_market_impact_single_path/{time_period_folder}/path_of_stock_prices_effect_of_market_impact_{time_period}_persistence_{persistence:.1f}.jpg".format(time_period_folder=time_period, time_period=time_period, persistence=persistence))

        fig = plt.figure(figsize=(10, 5))
        plt.plot(deltas_DH, label="Delta hedge")
        for i, global_deltas in enumerate(global_hedge_deltas):
            plt.plot(global_deltas, label="Global {loss} - {impact}".format(loss=loss_type, impact="no impact" if impact_values[i] == 0.0 and impact_values == 0.0 else "alpha = {:.4f}, beta = {:.4f}".format(1.0-impact_values[i], 1.0+impact_values[i])))
        plt.xlabel('Time Steps')
        ...
        plt.savefig("code_pytorch/effect_of_market_impact_single_path/{time_period_folder}/path_of_deltas_effect_of_market_impact_{time_period}_persistence_{persistence:.1f}.jpg".format(time_period_folder=time_period, time_period=time_period, persistence=persistence))
        ...
        for i, diff_deltas in enumerate(diff_global_deltas):
            plt.plot(diff_deltas, label="Global {loss} - {impact}".format(loss=loss_type, impact="no impact" if impact_values[i] == 0.0 and impact_values == 0.0 else "alpha = {:.4f}, beta = {:.4f}".format(1.0-impact_values[i], 1.0+impact_values[i])))
        plt.xlabel('Time Steps')
        ...
        plt.savefig("code_pytorch/effect_of_market_impact_single_path/{time_period_folder}/path_of_diff_deltas_effect_of_market_impact_{time_period}_persistence_{persistence:.1f}.jpg".format(time_period_folder=time_period, time_period=time_period, persistence=persistence))
