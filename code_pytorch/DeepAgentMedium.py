...
class DeepAgent():
    ...
    def __init__(self, nbs_point_traj, batch_size, r_borrow, r_lend, stock_dyn, params_vect, S_0, T, alpha, beta,
        ...
        self.model = FFNN(in_features=5, num_layers=nbs_layers, hidden_size=nbs_units, dropout=dropout).to(self.device)
    ...
    def simulate_batch(self, test_path=None):
        ...
        for t in range(self.N):
            ...
            input_t = torch.stack((self.dt * t * torch.ones(self.batch_size, device=self.device), self.S_t, self.delta_t, A_t, B_t), dim=1)
