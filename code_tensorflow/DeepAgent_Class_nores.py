...
class DeepAgent(object):
    ...
    def __init__(self, nbs_point_traj, batch_size, r_borrow, r_lend, stock_dyn, params_vect, S_0, T, alpha, beta,
        ...
        if (self.prepro_stock == "Log"):
            ...
            self.S_t = tf.math.log(S_0 / strike) * tf.ones(self.batch_size)
        ...
        for t in range(self.N):
            ...
            # input_t[i, :] = [S_t, delta_t, V_t] is the basis
            # - S_t and V_t are normalize already
            ...
            if (t == 0):
                ...
                cashflow = self.liquid_func(self.S_t, -delta_t_next[:, 0])
            ...
            # 5.5) Update features for next time-step
            ...
            L_t = self.liquid_func(self.S_t, self.delta_t)
        ...
        if (self.position_type == "Short"):
            if (self.option_type == "Call"):
                self.condition = tf.greater_equal(self.cost_selling(self.S_t, self.nbs_shares),
                                                  self.nbs_shares * self.strike)
                self.hedging_gain = tf.where(self.condition, self.Y_t + self.liquid_func(self.S_t,
                                                                                         self.delta_t - self.nbs_shares) + self.nbs_shares * self.strike,
                                             self.Y_t + self.liquid_func(self.S_t, self.delta_t))
            ...
            # - i.e. K = 100, S_T = 100.01, maybe will be able to sell only at 99.99 with market impact (execute)
                ...
                self.condition = tf.less_equal(self.cost_selling(self.S_t, self.nbs_shares),
                                               self.nbs_shares * self.strike)
                self.hedging_gain = tf.where(self.condition, self.Y_t + self.liquid_func(self.S_t,
                                                                                         self.delta_t + self.nbs_shares) - self.nbs_shares * self.strike,
                                             self.Y_t + self.liquid_func(self.S_t, self.delta_t))
    ...
    def liquid_func(self, S_t, x):
        return self.cost_selling(S_t, tf.nn.relu(x)) - self.cost_buying(S_t, tf.nn.relu(-x))
    ...
    def predict(self, sample_size, sess, epochs):
        ...
        for i in range(int(sample_size / batch_size)):
            ...
            hedging_err, strategy, S_t_tensor, V_t_tensor, input_t_tensor = sess.run(
                [self.hedging_err, self.strategy, self.S_t_tensor, self.V_t_tensor, self.input_t_tensor])
        ...
        return np.concatenate(strategy_pred, axis=1), np.concatenate(hedging_err_pred), np.concatenate(S_t_tensor_pred,
                                                                                                       axis=1), np.concatenate(
            V_t_tensor_pred, axis=1), np.concatenate(input_t_tensor_pred, axis=1)
