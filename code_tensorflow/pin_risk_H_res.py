...
ai_is_here = r"/home/clarence/UQAM/Research/Global_Hedging_LOB_projet_Fred_Alex/Code_Alexandre/Pycharm_version/Turnover/%s/models.ckpt"
...
if (option_type == 'Call'):
    ...
    V_0 = Utils_general.BlackScholes_price(S_0, T, r_borrow, sigma, strike, -1)
    #V_0 = V_0 - 0.5*V_0 #To cimport warnings
warnings.simplefilter(action='ignore', category=FutureWarning)
import datetime as dt
import numpy as np
#import tensorflow as tf             # TENSORFLOW ---> deep learning (installation CPU)
...
# hex if BS initial value is too low
...
fig.suptitle('Pin risk, high resilience')
...
plt.legend(legend)
plt.title('Turnover series, high resilience')
plt.xlabel('time')
plt.ylabel('turnover')
