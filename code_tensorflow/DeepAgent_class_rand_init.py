...
class DeepAgent(object):
    ...
    def __init__(self, nbs_point_traj, batch_size, r_borrow, r_lend, stock_dyn, params_vect, S_0, T, alpha, beta,
        ...
        self.A_0 = 2 * tf.random.uniform(shape=[self.batch_size], dtype=tf.dtypes.float32)  #Random initial value of the impact# 0 * tf.ones(self.batch_size) # Fixed init value #
        self.ha = 1 + 0.008 * np.random.uniform()  #Resilience parameter, ask part
        self.B_0 = 2 * tf.random.uniform(shape=[self.batch_size], dtype=tf.dtypes.float32) #Random initial value of the impact# 0 * tf.ones(self.batch_size) # Fixed init value #
        self.hb = 1 - 0.008 * np.random.uniform() #Resilience dparameter, bid part
        ...
        if (self.position_type == "Long"):
            V_t = -self.V_0 + 2 * (tf.random.uniform(shape=[self.batch_size], dtype=tf.dtypes.float32) - 0.5) # Random initial value centered on -self.V0 #-self.V_0 * tf.ones(self.batch_size) # Fixed init value
        ...
        for t in range(self.N):
            ...
            if (self.alpha == 1.00 and self.beta == 1.00):  # no impact, don,t include 'delta_t'
                input_t = tf.concat([tf.expand_dims(self.S_t, axis=1), tf.expand_dims(self.delta_t, axis=1),
                                     tf.expand_dims(tf.divide(V_t , self.V_0), axis=1)], axis=1)
            else:
                input_t = tf.concat([tf.expand_dims(self.S_t, axis=1), tf.expand_dims(self.delta_t, axis=1),
                                     tf.expand_dims(tf.divide(V_t, self.V_0), axis=1), tf.expand_dims(A_t, axis=1),
            ...
            if (t == 0):
                self.strategy = tf.expand_dims(delta_t_next, axis=0)  # [1, batch_size, 1]
                cashflow = self.liquid_func(self.S_t, -delta_t_next[:, 0], A_t, B_t)
                self.Y_t = V_t + cashflow  # time-0 amount in the bank account
                #Compute impact and resilience
                # ha = [-1, -1] is the value for no-impact model
                if self.ha == -1:
                    A_t = tf.constant(0, dtype=tf.float32, shape=delta_t_next[:, 0].shape)
                else:
                    impact_ask = tf.cast(tf.greater(delta_t_next[:, 0], tf.zeros(self.batch_size)), tf.float32) * delta_t_next[:, 0]
                    A_t = (A_t + impact_ask) * tf.exp(-self.ha * self.h)  # Update impact with

                    # transaction and include resilience for one period.
                if self.hb == -1:
                    B_t = tf.constant(0, dtype=tf.float32, shape=delta_t_next[:, 0].shape)
                else:
                    impact_bid = tf.cast(tf.less(delta_t_next[:, 0], tf.zeros(self.batch_size)), tf.float32) * delta_t_next[:, 0] *(-1)
                    B_t = (B_t + impact_bid) * tf.exp(-self.hb * self.h)  # Update impact with
                # transaction and include resilience for one period.

            else:
                self.strategy = tf.concat([self.strategy, tf.expand_dims(delta_t_next, axis=0)], axis=0)
                # Compute value in bank account
                diff_delta_t = delta_t_next[:, 0] - self.strategy[t - 1, :, 0]
            ...
            if (self.stock_dyn == "BSM"):
               # tf.compat.v1.set_random_seed(101) #### Uncomment to get the same random events sequence
    ...
    def _execute_graph_batchwise(self, sample_size_train, sess, epochs):
        ...
        while (epoch < epochs):
            ...
            for i in range(int(sample_size_train / self.batch_size)):
                # Training step
                _, strat_tmp, hedging_err = sess.run([self.train, self.strategy, self.hedging_err])
          ...
          #  if (epoch + 1) % 1 == 0:
            ...
            if (self.loss_epochs[epoch] < loss_best):
                ...
                # Here to change the location of the AI
                self.saver.save(sess,
                                r"/home/clarence/UQAM/Research/Global_Hedging_LOB_projet_Fred_Alex/Code_Alexandre/Pycharm_version/TEST/%s/models.ckpt" % self.model_name)
    ...
    def predict(self, sample_size, sess, epochs):
        ...
        ret1 = np.concatenate(strategy_pred, axis=1)
        ret2 = np.concatenate(hedging_err_pred)
        ret3 = np.concatenate(S_t_tensor_pred, axis=1)
        ret4 = np.concatenate(V_t_tensor_pred, axis=1)
        ret5 = np.concatenate(A_t_tensor_pred, axis=1)
        ret6 = np.concatenate(B_t_tensor_pred, axis=1)
        ret7 = np.concatenate(input_t_tensor_pred, axis=1)
